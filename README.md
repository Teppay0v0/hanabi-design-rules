...
├── README.md                 ← このファイル
└── references/               ← テンプレート集（全ルール実装済み）
    ├── lp-starter.html       ← 汎用LP
    ├── lp-restaurant.html    ← 飲食店
    ├── lp-salon.html         ← 美容室・サロン
    └── lp-professional.html  ← 士業・コンサル
```

「居酒屋のLP作って」のように業種を伝えるだけで、Claude が該当スターターをベースに作ります。
どれも `:root` の `--main` `--sub` `--wash` を変えるだけでブランド色に切り替わります。

**テンプレート一覧デモ:** https://teppay0v0.github.io/hanabi-design-rules/#templates
